  ...
  private submit = (
    ...
    timeLog({
      update: (store, { data: { createTimeLog } }) => {
        const {
          me: { id }
        }: any = store.readQuery({ query: ME });
        const { timeLogsRange }: any = store.readQuery({
          query: TIME_LOGS_RANGE,
          variables: { ...this.props.variables, userId: id }
        });
        store.writeQuery({
          query: TIME_LOGS_RANGE,
          variables: { ...this.props.variables, userId: id },
          data: { timeLogsRange: timeLogsRange.concat(createTimeLog) }
        });
      },
      variables: { ...model }
    });
